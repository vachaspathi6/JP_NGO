  ...
  const fetchRecommendedBooks = async (studentId) => {
    try {
      ...
      const booksWithImages = recommendedResults.map(book => ({
        ...book,
        imageUrl: `https://via.placeholder.com/150?text=${encodeURIComponent(book.Book_Name)}`, // Replace this with the actual image URL from Google
      }));
